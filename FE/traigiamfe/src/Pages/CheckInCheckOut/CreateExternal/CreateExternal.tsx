...
const CreateExternal = (props: ICreateExternal) => {
    ...
    const handleConfirm = async () => {
        try {
            ...
            await axios.put(
                `https://localhost:7120/api/External/${currentRecord?.id}/confirm`, model
            );
    ...
    const renderContent = (status: number) => {
        switch (status) {
            case 0:
                return <div>Bạn có chấp nhận cho phạm nhân <span style={{ fontWeight: '600' }}>{currentRecord?.prisonerName}</span> ra ngoài.</div>
            case 1:
                return <div>Nhân nút ra ngoài để phạm nhân <span style={{ fontWeight: '600' }}>{currentRecord?.prisonerName}</span> hoàn thành xác nhận ra ngoài.</div>
            case 2:
                return <div>Nhân nút vào trong để xác nhận phạm nhân <span style={{ fontWeight: '600' }}>{currentRecord?.prisonerName}</span> đã trở lại.</div>
            default:
                break;
        }
    }
