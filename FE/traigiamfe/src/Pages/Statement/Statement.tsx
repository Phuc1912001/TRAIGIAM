...
const Statement = () => {
  ...
  const [dataOriginStatement, setDataOriginStatement] = useState<
    StatmentModel[]
  >([]);
  const [openCreateStatement, setOpenCreateStatement] =
  ...
  const handleGetAllStatement = async () => {
    try {
      ...
      setDataOriginStatement(data.data);
  ...
  const onSearch = (val: string) => {
    if (val.trim() === "") {
      setDataStatement(dataOriginStatement);
    } else {
      const newList = dataOriginStatement.filter((item: StatmentModel) =>
        item?.prisonerName?.toLowerCase().includes(val.toLowerCase())
      );
      setDataStatement(newList);
    }
  };
